...
class ListTvShowAdapter : RecyclerView.Adapter<ListTvShowAdapter.ListViewHolder>() {
    private val castList = ArrayList<TvShowCreditsEntity>()

    fun setCastList(list: List<TvShowCreditsEntity>)
    {
        castList.clear()
    ...
    inner class ListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(castDetails: TvShowCreditsEntity) {
            with(itemView) {
    ...
    interface OnItemClickCallback {
        fun onItemClicked(castDetails: TvShowCreditsEntity)
    }
}